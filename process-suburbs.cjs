...
const zonedFeatures = sydneyFeatures.map(feature => {
  ...
  // Simple geographic-based zoning
  // Eastern and Northern Sydney (generally more expensive)
  if (feature.geometry.type === 'Polygon') {
    ...
    // Eastern/Northern Sydney (green zone)
    if (centerLng > 151.1 && centerLat < -33.7) {
      zone = 'green';
    }
    // Western Sydney (red zone)
    else if (centerLng < 150.9) {
      zone = 'red';
    }
  }
  
  // Override with specific suburb lists if needed
  ...
  return {
    ...
    properties: {
      ...feature.properties,
      zone,
      // Add some random metadata for demo purposes
      postcode: Math.floor(2000 + Math.random() * 999).toString(),
      population: Math.floor(5000 + Math.random() * 45000),
      median_income: Math.floor(60000 + Math.random() * 140000),
      area_sqkm: Math.floor(1 + Math.random() * 20 * 100) / 100
    }
  };
});
